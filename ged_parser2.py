...
class Sprint1:
    ...
    individuals_dict, families_dict, individuals_age, is_alive, duplicate_name, individual_births, marriageDate = {}, {}, {}, {}, {}, {}, {}
    ...
    def sameNameSameBirthday(self):
        for keys in self.duplicate_name:
            if keys in self.individual_births:
                self.nameBirthday.append(keys)
        return self.nameBirthday
    ...
    def compareNamesAndBirth(self,siblings, final):
        ...
        for sibling in siblings:
            name = self.individuals_dict.get(sibling)
            birthday = self.individual_births.get(sibling)
            siblingName.append(name)
            siblingBirth.append(birthday)
            
        if 2 in Counter(siblingName).values()and 2 in Counter(siblingBirth).values():
            final.append(siblings)
        ...
        if children:
            for child in children:
                ...
                if child.get_tag() == "NAME":
                    ...
                    name = name[0]
                    indi_values = self.individuals_dict.values()
                    dict_items=self.individuals_dict.items()
                    # dupe_keys = self.duplicate_name.keys()
                    if name in indi_values:
                        previouslyPresent=[key for key,value in dict_items if value==name]
                        self.duplicate_name[''.join(previouslyPresent)] = name
                        self.duplicate_name[ID] = name
    ...
    def family_helper(self,element, fID):
        ...
        if children:
            ...
            if married in self.marriageDate:
                if married != "NA":
                    self.couple.append(self.marriageDate[married])
                    self.couple.append([hID,wID])
            else:
                self.marriageDate[married] = [hID,wID]
            if len(spawns)>1:
                self.Children.append(spawns)
