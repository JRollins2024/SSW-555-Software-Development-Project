...
def ageDifference(hID, wID):  
    """ This function returns the couples whose age difference is huge """
    ageDifferenceArray = []
    husbandAge  = individuals_age.get(hID)
    wifeAge = individuals_age.get(wID)
    ages = [husbandAge, wifeAge]
    if husbandAge and wifeAge and max(ages)/min(ages) > 2:
        husbandName = individuals_dict.get(hID)
        wifeName = individuals_dict.get(wID)
        # print(husbandAge, husbandName, wifeAge, wifeName)
        ageDifferenceArray.append(hID)
        ageDifferenceArray.append(wID)
    return ageDifferenceArray

def orphans(hID, wID, chil):
    aliveFather = is_alive.get(hID)
    aliveMother = is_alive.get(wID)
    if (aliveFather == 'False' or aliveFather == None) and (aliveMother == 'False' or aliveMother == None):
        if individuals_age.get(chil) < 18:
           orphansUnder18.append(chil) 
    ...
    if children:
        for child in children:
            ...
            if child.get_tag() == "WIFE":
                ...
                wID = wID.splitlines()
                wID = wID[0]
                ageDifferenceVar = ageDifference(hID, wID)
                if ageDifferenceVar:
                    SpouseTwiceTheAge.append(ageDifferenceVar)
...
print("Orphaned children (both parents dead and child < 18 years old) in a GEDCOM file", orphansUnder18)

print("Couples who were married when the older spouse was more than twice as old as the younger spouse", SpouseTwiceTheAge)
